from mcs_envs.crazy_env.crazy_env_setting import Setting
# from mcs_envs.env_setting.env_setting_20 import Setting
...
class Env(gym.Env):
    def __init__(self, log=None):
        ...
        # [[2]]
        # TODO:重大修改，连续动作离散化
        self.action_space = spaces.Box(low=-1, high=1, shape=(self.sg.V['NUM_UAV'] * self.sg.V['NUM_ACTION'],))
    ...
    def reset(self):
        ...
        self.uav = [list(self.sg.V['INIT_POSITION']) for i in range(self.num_uav)]
        self.eff = [0.] * self.num_uav
        self.count = 0
        self.zero = 0

        self.trace = [[] for i in range(self.num_uav)]
        self.energytrace = [[] for i in range(self.num_uav)]
        # initialize remaining energy
        self.energy = np.ones(self.num_uav).astype(np.float32) * self.maxenergy
        # initialize indicators
        self.collection = np.zeros(self.num_uav).astype(np.float32)
        # energy use
        self.use_energy = np.zeros(self.num_uav).astype(np.float32)
        # energy fill
        self.fill_energy = np.zeros(self.num_uav).astype(np.float32)
        # energy max
        self.max_energy_array = np.array([self.maxenergy] * self.num_uav).astype(np.float32)
    ...
    def __init_image(self):
        ...
        for width in range(image.shape[0]):
            for height in range(image.shape[1]):
                ...
                image[1][width][height] = self.image_position[width][height]
                image[2][width][height] = self.image_access[width][height]
        state = image
        return state
    ...
    def step(self, actions, indicator=None):
        ...
        reward = [0] * self.num_uav
        self.tmp_value = [0] * self.num_uav
        self.tmp_energy = [0] * self.num_uav
        self.tmp_distance = [0] * self.num_uav
        self.tmp_penalty = [0] * self.num_uav
        self.dn = [False] * self.num_uav  # no energy UAV
        ...
        info = {"performance_info": {
            ...
            "normal_fairness": self.normal_collection_fairness,
            "use_energy": np.sum(self.normal_use_energy),
        ...
        return state, np.sum(self.reward), done, info

    def render(self, mode='human'):
        global power_list
        observ = self.__get_state()
        observ = observ.transpose((2, 1, 0))
        observ_0 = observ[:, :, 0]
        observ_1 = observ[:, :, 2]
