...
class Log(object):
    def __init__(self, level_name, dir):
        self.time = str(time.strftime("%Y  %m-%d  %H-%M-%S", time.localtime()))
        self.level_name = level_name
        self.full_path = os.path.join(dir, self.level_name)
        self.choose_color = ['blue', 'green', 'purple', 'red']

        # TODO:DEBUG
        debug_store_path = os.path.join(self.full_path, 'img_debug')
        if not os.path.exists(debug_store_path):
            os.makedirs(debug_store_path, exist_ok=True)
    ...
    def draw_path(self, env, env_i, meaningful_fill, meaningful_get):
        ...
        for i in range(env.num_uav):
            ...
            plt.ylim(bottom=0, top=env.mapy)
            plt.xlim(left=0, right=env.mapx)
            ...
            plt.title('Meaningful Get:' + str(meaningful_get) + '\nMeaningful Fill:' + str(
                meaningful_fill) + '\nLeft Reward=' + str(round(env.leftrewards, 3)) + ',Efficiency=' + str(
                round(env.efficiency, 3)))
